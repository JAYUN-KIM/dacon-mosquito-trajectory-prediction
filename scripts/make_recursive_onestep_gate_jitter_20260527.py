...
PUBLIC_WINNER = "recstep_rank4_gate_osc89b005late_f100s100u100_top080_b40.csv"
PUBLIC_WINNER_SCORE = 0.69180
...
CONFIGS = [
    # Safest local jitter around the public-positive top080_b40 point.
    JitterConfig("top080_b425", 0.080, 0.425),
    JitterConfig("top075_b425", 0.075, 0.425),
    JitterConfig("top085_b425", 0.085, 0.425),
    JitterConfig("top080_b375", 0.080, 0.375),
    JitterConfig("top080_b450", 0.080, 0.450),
    # Direction-finding variants: does public want narrower/wider or stronger?
    JitterConfig("top070_b400", 0.070, 0.400),
    JitterConfig("top090_b400", 0.090, 0.400),
    JitterConfig("top070_b450", 0.070, 0.450),
    JitterConfig("top090_b450", 0.090, 0.450),
    JitterConfig("top065_b425", 0.065, 0.425),
    JitterConfig("top095_b425", 0.095, 0.425),
    JitterConfig("top080_b475", 0.080, 0.475),
]
...
def output_name(rank: int, config: JitterConfig) -> str:
    return (
        f"recstepgate27_rank{rank}_{config.name}_{mult_slug(WINNER_MULT)}_"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="2026-05-27 jitter around the public-winning recursive one-step gate.")
    ...
    parser.add_argument("--report-path", type=Path, default=ROOT / "reports" / "latest_recursive_onestep_gate_jitter_20260527.md")
...
def main() -> None:
    ...
    winner_candidate = Candidate("gate", SPEC_NAME, WINNER_MULT, 0.40, frac=0.080)
    winner_oof = apply_candidate(base_champion_oof, rec_oof, winner_candidate, gain_oof)
    winner_oof_hit = distance_summary(winner_oof, y)["r_hit_1cm"]
    ...
    for config in CONFIGS:
        ...
        row = {
            ...
            "oof_delta_vs_public_winner_proxy": metrics["r_hit_1cm"] - winner_oof_hit,
            **delta_summary(pred_oof, winner_oof, "oof_vs_winner"),
            **penalty,
        }
        # Public prior: stay close to the known 0.69180 winner unless OOF gain is clearly better.
        row["selection_score"] = (
            0.65 * row["oof_delta_vs_base_champion"]
            + 0.35 * row["oof_delta_vs_public_winner_proxy"]
            - 0.08 * max(0.0, row["vs_public_winner_mean_delta"] - 0.00008)
            - 0.00008 * max(0, row["changed_vs_public_winner"] - 900) / 100.0
        )
        rows.append(row)
        ...
        output_rows.append(
            {
                ...
                "oof_delta_vs_public_winner_proxy": float(row["oof_delta_vs_public_winner_proxy"]),
